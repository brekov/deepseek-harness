...
function runPnpm(
  ...
  // npm_execpath names either a JS entry to run under this Node or a native pnpm binary.
  ...
  if (run !== undefined) return run.run(args.join(' '), command, commandArgs, { cwd, env })
